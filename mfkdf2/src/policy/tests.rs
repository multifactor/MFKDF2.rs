...
async fn policy_derivation_combinations(
  ...
  let factors: Vec<_> =
    factor_names.iter().zip(ids.iter()).map(|(n, id)| create_policy_factor(n, id)).collect();
  ...
  // Build a mapping from factor name to actual policy id by matching on kind and order
  let mut name_id_map = std::collections::HashMap::new();
  let mut used = vec![false; setup.policy.factors.len()];
  for (name, id) in factor_names.iter().zip(ids.iter()) {
    // Find the first unused policy factor with the same kind as the setup factor
    let setup_kind = create_policy_factor(name, id).kind();
    let mut found = false;
    for (i, pf) in setup.policy.factors.iter().enumerate() {
      if !used[i] && pf.kind == setup_kind {
        name_id_map.insert(*name, pf.id.clone());
        used[i] = true;
        found = true;
        break;
      }
    }
    assert!(found, "Could not find policy factor for name {} and kind {}", name, setup_kind);
  }
    ...
    for _ in 0..derivation_runs {
      let derive_factors: HashMap<_, _> = combo
        ...
        .map(|name| {
          ...
          create_policy_derive_factor(name, id, &setup.policy)
        })
        .collect();
