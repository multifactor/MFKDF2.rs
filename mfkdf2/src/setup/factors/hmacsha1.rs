...
//!
//! At a high level:
//! - during **setup**, you start from a 160‑bit HMAC key `k_F` (optionally generated by MFKDF2),
//!   wrap it in a `HmacSha1` factor, and have MFKDF2 produce an initial challenge + encrypted pad
//!   that can later be stored with the policy;
//! - during **derive**, the policy provides a fresh challenge for this factor; your application
//!   sends that challenge to the hardware token, collects the 20‑byte HMAC‑SHA1 response, and
//!   passes it back to MFKDF2 as [`HmacSha1Response`], which lets the library recover the same
//!   secret key material without the token ever exposing `k_F` again.
//!
...
///   and the token agree on the same `k_F`.
...
///
/// The factor is consumed by the derive side via [`crate::derive::factors::hmacsha1`] which accepts
/// an [`HmacSha1Response`] (the 20‑byte HMAC‑SHA1 output from the hardware token for the current
/// challenge) and reconstructs the original secret.
///
/// # Errors
/// - [`MFKDF2Error::MissingFactorId`] if `id` is provided but empty.
/// - [`MFKDF2Error::InvalidSecretLength`] if `secret` is supplied with a length other than 20
///   bytes.
///
/// When driving this factor through [`crate::derive::key`], derivation may additionally fail with:
/// - [`MFKDF2Error::MissingDeriveParams`] if the persisted policy params for this factor are
///   missing the `"pad"` field;
/// - [`MFKDF2Error::InvalidDeriveParams`] if those params are malformed or cannot be decoded (for
///   example, if `"pad"` has been tampered with).
///
/// # Example
///
/// ```rust
